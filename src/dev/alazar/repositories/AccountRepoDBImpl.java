...
import dev.alazar.models.Account;
import dev.alazar.util.JDBCCon;
...
public class AccountRepoDBImpl implements AccountDao {
	...
	public Account updateAccount(Account change) {
		String sql = ("UPDATE accounts SET balance = ?, amount = ?, checkingAccount = ?, savingsAccount = ? WHERE id = ? and clientId = ? ");
		try {
	...
	public Account withdraw(Account a, Account c) {
		String sql = ("UPDATE accounts SET balance=balance-?, amount = -? WHERE id = ? and clientId = ? ");
	...
	public Account deleteAccount(int id, int id2) {
		...
		// Instructions for database
		String sql = "DELETE FROM accounts WHERE id = ? and clientId = ? RETURNING *";
	...
	private Account buildAccount(ResultSet rs) throws SQLException {
		...
		aa.setCid(rs.getInt("clientId"));
		aa.setBalance(rs.getInt("balance"));
		aa.setAmount(rs.getInt("amount"));
		aa.setCheckingAccount(rs.getBoolean("checkingAccount"));
		aa.setSavingsAccount(rs.getBoolean("savingsAccount"));
		return aa;
	}
}
